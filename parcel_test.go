...
func TestSetAddress(t *testing.T) {
	...
	assert.Equal(t, parcel.Status, updatedParcel.Status) // статус не должен измениться
}
...
func TestSetStatus(t *testing.T) {
	...
	assert.Equal(t, parcel.Address, updatedParcel.Address) // адрес не должен измениться
}
...
func TestGetByClient(t *testing.T) {
	...
	for _, parcel := range storedParcels {
		...
		assert.True(t, exists, "посылка под номером %d не найдена", parcel.Number)
